...
function build(target) {
  ...
  const sourceFpPath = path.join(__dirname, '../../fp');
  const resolvedSource = path.resolve(sourceFpPath);
  const resolvedDest = path.resolve(fpPath);
  if (resolvedSource !== resolvedDest) {
    actions.unshift(file.copy(sourceFpPath, fpPath));
  }
